...
**Before:** DD014 Phase 1 uses marching cubes to reconstruct a smooth body surface from SPH particles. The 688 Virtual Worm meshes sit as static, undeformed geometry inside a moving body — completely wrong.
...
6. **Backward compatibility.** When `visualization.mesh_deformation: false` in `openworm.yml`, the viewer falls back to marching-cubes surface reconstruction (DD014 Phase 1). No regression in existing functionality.
