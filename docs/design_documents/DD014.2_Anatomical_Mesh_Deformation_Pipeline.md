...
**Date:** 2026-02-16  
**Companion to:** [DD014](DD014_Dynamic_Visualization_Architecture.md) (Dynamic Visualization Architecture), [DD014.1]([DD014](DD014_Dynamic_Visualization_Architecture.md).1_Visual_Rendering_Specification.md) (Visual Rendering Specification)  
...
**Before:** [DD014](DD014_Dynamic_Visualization_Architecture.md) Phase 1 uses marching cubes to reconstruct a smooth body surface from SPH particles. The 688 Virtual Worm meshes sit as static, undeformed geometry inside a moving body — completely wrong.
...
6. **Backward compatibility.** When `visualization.mesh_deformation: false` in `openworm.yml`, the viewer falls back to marching-cubes surface reconstruction ([DD014](DD014_Dynamic_Visualization_Architecture.md) Phase 1). No regression in existing functionality.
...
3. **Growth and developmental changes.** Body size and shape change during larval development (L1-L4 → adult). [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) assumes a fixed adult body. Developmental morphing is Phase 6 work.

4. **Real-time SPH simulation.** [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) consumes pre-recorded SPH output. It does not run Sibernetic in real-time.

5. **Mesh generation or repair.** [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) assumes the Virtual Worm meshes are valid and watertight. If mesh defects exist (holes, non-manifold edges, self-intersections), they must be repaired upstream.
...
7. **Color, lighting, and rendering.** See [DD014.1]([DD014](DD014_Dynamic_Visualization_Architecture.md).1_Visual_Rendering_Specification.md). [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) is purely about geometry deformation.
...
For 10 years, the Virtual Worm meshes and the Sibernetic simulation have existed as parallel representations with no bridge between them. [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) closes this gap.
...
The tools exist. The anatomical data exists. [DD014.2]([DD014](DD014_Dynamic_Visualization_Architecture.md).2_Anatomical_Mesh_Deformation_Pipeline.md) specifies how to connect them.
...
|---------------|----|-----------------------------|
| Visual rendering | [DD014](DD014_Dynamic_Visualization_Architecture.md) / [DD014.1]([DD014](DD014_Dynamic_Visualization_Architecture.md).1_Visual_Rendering_Specification.md) | If deformed vertex buffer format changes, the renderer must update its vertex attribute bindings |
...
| Activity coloring | [DD014.1]([DD014](DD014_Dynamic_Visualization_Architecture.md).1_Visual_Rendering_Specification.md) | Activity color overlay is applied to deformed mesh vertices — deformation must preserve vertex-to-cell mapping (vertex indices must not change) |
