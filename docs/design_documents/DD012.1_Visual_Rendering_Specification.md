# DD012.1: Visual Rendering Specification for Multi-Scale Visualization

- **Status:** Proposed
- **Author:** OpenWorm Core Team
- **Date:** 2026-02-16
- **Companion to:** [DD012](DD012_Dynamic_Visualization_Architecture.md) (Dynamic Visualization and Multi-Scale Exploration Architecture)
- **Related:** [DD002](DD002_Neural_Circuit_Architecture.md) (Neural Circuit), [DD003](DD003_Muscle_Model_Architecture.md) (Muscle Model), [DD001](DD001_Body_Physics_Architecture.md) (Body Physics), [DD004](DD004_Mechanical_Cell_Identity.md) (Cell Identity), [DD005](DD005_Cell_Type_Differentiation_Strategy.md) (Cell-Type Specialization), [DD006](DD006_Neuropeptidergic_Connectome_Integration.md) (Neuropeptidergic Connectome), [DD007](DD007_Pharyngeal_System_Architecture.md) (Pharyngeal System), [DD008](DD008_Data_Integration_Pipeline.md) (Data Integration), [DD009](DD009_Intestinal_Oscillator_Model.md) (Intestinal Oscillator), [DD010](DD010_Validation_Framework.md) (Validation Framework)

---

> **Phase:** [Phase 3](DD_PHASE_ROADMAP.md#phase-3-cell-type-differentiation) | **Layer:** Visualization

## TL;DR

This document defines what the OpenWorm simulation **looks like** — exact colors, materials, lighting, transparency, transitions, and rendering algorithms for every visual element at every scale. [DD012](DD012_Dynamic_Visualization_Architecture.md) specifies the architecture (data pipeline, Zarr schema, Trame/Three.js framework, Docker integration); [DD012.1](DD012.1_Visual_Rendering_Specification.md) specifies the visual design. A developer should be able to implement any visual element by reading [DD012.1](DD012.1_Visual_Rendering_Specification.md) (appearance) and [DD012](DD012_Dynamic_Visualization_Architecture.md) (data format) without guessing.
...
- Theme/style configuration file (JSON/YAML) for the DD012 viewer `[TO BE CREATED]`

## Repository & Issues

- Primary repository: `openworm/viewer` `[TO BE CREATED]` (rendering configuration and theme files)
- Issue label: `dd012.1`, `rendering`
- Related: [DD012](DD012_Dynamic_Visualization_Architecture.md) parent DD (viewer architecture)

## How to Build & Test

- **Prerequisites:** DD012 viewer running locally (see [DD012](DD012_Dynamic_Visualization_Architecture.md))
...
|-------------|------------------------|------------|-------------|----------|-------------|-----------|
| Neuron membrane voltage | Brightness + hue shift | Base color at 30% brightness | Base color at 100% brightness + white core glow | Blue tint (rest) → base color (threshold) → white core (peak) | -80 mV (rest) to +20 mV (peak depolarization) | [DD002](DD002_Neural_Circuit_Architecture.md) |
| Neuron intracellular [Ca²⁺] | Additive glow color | No glow (black additive) | Gold/yellow glow (`#FFD700`) | Black (0) → Gold (max) additive blend | 0 to `max_ca` (per-neuron normalized) | [DD002](DD002_Neural_Circuit_Architecture.md) |
| Muscle activation | Hue shift (green→red) | Base green color (`#295200`) | Red (`#CC2200`) | `RdYlGn_r` (reversed Red-Yellow-Green) | 0.0 (relaxed) to 1.0 (max contraction) | [DD003](DD003_Muscle_Model_Architecture.md) |
...
| SPH particle type | Point color (categorical) | N/A | N/A | Blue=liquid, Green=elastic, Gray=boundary | Categorical | [DD001](DD001_Body_Physics_Architecture.md) |
...
| Property | WebGPU (Three.js Phase 4/3) | WebGL Fallback (Phase 3 / Trame) |
|----------|---------------------------|----------------------------------|
...
|----------|-------|-------|
| Geometry source | Marching cubes isosurface from SPH particles (Phase 3); VirtualWorm mesh skinning (Phase 4+) | See [DD012](DD012_Dynamic_Visualization_Architecture.md) Surface Reconstruction |
...
| Muscle ridges (Phase 4+) | Subtle normal-map perturbation on dorsal/ventral muscle quadrants | Amplitude 0.02 mm, 24 ridges per quadrant matching BWM count |
...
| Individual cell segmentation | [DD004](DD004_Mechanical_Cell_Identity.md) particle-to-cell mapping → convex hull per cell (Phase 4+) | Phase 3 uses tissue-level meshes, not individual cells |
...
| Striations (Phase 4+) | Normal map: parallel ridges along muscle long axis | Period 2 µm, amplitude 0.01 mm, visible only at close zoom |
...
| Grinder visualization (Phase 5) | Rotation animation of terminal bulb interior | Simplified: sinusoidal rotation ±15° synchronized with pump cycle |
...
| Microtubule hints (Phase 5) | Faint gray lines (`#333340`) from nucleus toward cell periphery | Not individually modeled; subtle visual guides suggesting cytoskeletal structure. Vesicles follow these paths. |
...
|-----------|----------|---------------|
| Order-Independent Transparency (OIT) | WebGPU path (Phase 4/3) | Weighted Blended OIT (McGuire & Bavoil 2013). Weight function: `w = alpha * max(0.01, min(3000, 10 / (0.00001 + pow(z/200, 4))))` where z is view-space depth. |
| Depth-sorted alpha blending | WebGL/Trame fallback (Phase 3) | Sort transparent objects back-to-front per frame. Acceptable for < 100 transparent objects. |
...
| Temporal AA | WebGPU path (Phase 5) for smoother animation |
...
| Minimum acceptable | 15 fps | On 2020-era laptop with integrated GPU (per [DD012](DD012_Dynamic_Visualization_Architecture.md) QC5) |
...
![Mockup 1: Organism dorsal view](../images/mockup_01_organism_dorsal.png)
...
![Mockup 2: Organism lateral view](../images/mockup_02_organism_lateral.png)
...
![Mockup 3: Muscles through translucent body](../images/mockup_03_muscles_translucent.png)
...
![Mockup 4: Neural circuit overview](../images/mockup_04_neural_circuit.png)
...
![Mockup 5: Single neuron selected with inspector panel](../images/mockup_05_neuron_inspector.png)
...
![Mockup 6: Pharynx close-up pumping](../images/mockup_06_pharynx_pumping.png)
...
![Mockup 7: Intestinal calcium wave](../images/mockup_07_intestinal_calcium.png)
...
![Mockup 8: Neuropeptide volumetric clouds](../images/mockup_08_neuropeptide_clouds.png)
...
![Mockup 9: Cell boundaries](../images/mockup_09_cell_boundaries.png)
...
![Mockup 10: Membrane cross-section with ion channels](../images/mockup_10_membrane_channels.png)
...
![Mockup 11: Calcium flowing through open channel](../images/mockup_11_calcium_flow.png)
...
![Mockup 12: Multi-scale transition](../images/mockup_12_scale_transition.png)
...
![Mockup 13: Nucleus and gene transcription](../images/mockup_13_nucleus_transcription.png)
...
![Mockup 14: mRNA export and channel assembly pipeline](../images/mockup_14_gene_to_channel_pipeline.png)
...
- **Data formats or schemas** — See [DD012](DD012_Dynamic_Visualization_Architecture.md) (OME-Zarr structure, `.zattrs` metadata)
- **Viewer UI layout or interaction design** — See [DD012](DD012_Dynamic_Visualization_Architecture.md) (layout mockup, layer toggle system, inspector panel)
- **Docker deployment or CI/CD** — See [DD012](DD012_Dynamic_Visualization_Architecture.md) + [DD011](DD011_Simulation_Stack_Architecture.md)
- **Simulation algorithms** — See [DD002](DD002_Neural_Circuit_Architecture.md)-[DD009](DD009_Intestinal_Oscillator_Model.md)
- **Validation criteria** — See [DD010](DD010_Validation_Framework.md)
- **Web framework choice (Trame vs Three.js)** — See [DD012](DD012_Dynamic_Visualization_Architecture.md)

[DD012.1](DD012.1_Visual_Rendering_Specification.md) specifies purely: what things look like, what color they are, how they glow, how they transition between scales, and what the rendering algorithms are.
...
2. **[DD012](DD012_Dynamic_Visualization_Architecture.md): Dynamic Visualization Architecture:** Companion document specifying data pipeline, Zarr schema, viewer framework, and Docker integration.
...
|-----------|----------------|------|--------|
| Input | [DD012](DD012_Dynamic_Visualization_Architecture.md) | Simulation state data | OME-Zarr |
| Output | Viewer | Rendered frames | WebGL/PNG |

### Configuration

- Color palette: defined as JSON/YAML theme file consumed by DD012 viewer
- Rendering parameters: ambient occlusion radius, SSS depth, bloom threshold

### Coupling Dependencies

- **Upstream:** [DD012](DD012_Dynamic_Visualization_Architecture.md) (viewer architecture), [DD005](DD005_Cell_Type_Differentiation_Strategy.md) (cell type list determines color assignments)
...
3. Implement heritage palette in Worm3DViewer (Phase 3) — replace current ad-hoc colors with Section 1A
...
7. Design data pipeline for gene expression events (transcription → export → translation → trafficking) — may require new Zarr group in [DD012](DD012_Dynamic_Visualization_Architecture.md) schema
