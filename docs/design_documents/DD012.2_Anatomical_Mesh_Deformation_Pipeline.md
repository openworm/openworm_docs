# DD012.2: Anatomical Mesh Deformation Pipeline

- **Status:** Proposed
- **Author:** OpenWorm Core Team
- **Date:** 2026-02-16
- **Companion to:** [DD012](DD012_Dynamic_Visualization_Architecture.md) (Dynamic Visualization Architecture), [DD012.1](DD012.1_Visual_Rendering_Specification.md) (Visual Rendering Specification)
- **Related:** [DD001](DD001_Body_Physics_Architecture.md) (Body Physics / Sibernetic), [DD004](DD004_Mechanical_Cell_Identity.md) (Mechanical Cell Identity), [DD011](DD011_Simulation_Stack_Architecture.md) (Simulation Stack)

---

> **Phase:** [Phase 6](DD_PHASE_ROADMAP.md#phase-6-mechanical-cell-identity-high-fidelity-visualization) | **Layer:** Visualization
...
| **Secondary:** Inter-mesh non-penetration | Adjacent organs (intestine, gonad, muscle quadrants) do not interpenetrate during body bending | Visual QC (non-blocking Phase 4) |
...
**Before:** [DD012](DD012_Dynamic_Visualization_Architecture.md) Viewer Stage 1 (Roadmap Phase 3) uses marching cubes to reconstruct a smooth body surface from SPH particles. The 688 Virtual Worm meshes sit as static, undeformed geometry inside a moving body — completely wrong.
...
### The Spine Shortcut (Optimization for Phase 3)

The Virtual Worm Blender file contains a **bezier spine curve** running along the body's central axis. The `Blender2NeuroML` repo already has `blenderStraighten.py` that uses it. This offers a fast Phase 3 approximation before the full cage pipeline is ready:
...
**Recommendation:** Use spine-based deformation for Phase 3 (gets meshes moving immediately), then upgrade to the full cage pipeline for Phase 4 (captures cross-sectional deformation and individual muscle bulging).
...
| Spine deformation shader | `shaders/spine_deform.wgsl` | WGSL | Phase 3 spine-based shortcut |
...
| **Branch convention** | `dd012b/description` (e.g., `dd012b/cage-deformation`, `dd012b/spine-shortcut`) |
| **Example PR title** | `DD012.2: Implement spine-based deformation for Phase 3 viewer` |
...
This path uses the pre-built deformation pipeline inside the Docker viewer service. See [DD011](DD011_Simulation_Stack_Architecture.md) for full Docker environment setup.
...
**This DD is itself a visualization pipeline.** Its output IS the deformed meshes rendered in the [DD012](DD012_Dynamic_Visualization_Architecture.md) viewer.

| Viewer Feature | Specification |
|---------------|---------------|
| **Default view** | Deformed Virtual Worm meshes replace marching-cubes surface at Phase 4+ |
...
| **A/B comparison** | Side-by-side: marching cubes surface (Phase 3) vs. deformed Virtual Worm meshes (Phase 4) |
...
6. **Backward compatibility.** When `visualization.mesh_deformation: false` in `openworm.yml`, the viewer falls back to marching-cubes surface reconstruction ([DD012](DD012_Dynamic_Visualization_Architecture.md) Viewer Stage 1). No regression in existing functionality.
...
1. **Muscle contraction visualization.** Visible muscle bulging/shortening during contraction is a Phase 5 enhancement requiring per-muscle deformation offsets driven by [DD003](DD003_Muscle_Model_Architecture.md) activation signals. [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) handles only the gross body-shape deformation.
...
3. **Growth and developmental changes.** Body size and shape change during larval development (L1-L4 → adult). [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) assumes a fixed adult body. Developmental morphing is Phase 8 work.

4. **Real-time SPH simulation.** [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) consumes pre-recorded SPH output. It does not run Sibernetic in real-time.

5. **Mesh generation or repair.** [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) assumes the Virtual Worm meshes are valid and watertight. If mesh defects exist (holes, non-manifold edges, self-intersections), they must be repaired upstream.
...
7. **Color, lighting, and rendering.** See [DD012.1](DD012.1_Visual_Rendering_Specification.md). [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) is purely about geometry deformation.
...
For 10 years, the Virtual Worm meshes and the Sibernetic simulation have existed as parallel representations with no bridge between them. [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) closes this gap.
...
The tools exist. The anatomical data exists. [DD012.2](DD012.2_Anatomical_Mesh_Deformation_Pipeline.md) specifies how to connect them.

---

## Implementation Roadmap

### Stage A: Spine-Based Deformation
...
| Task | Effort | Dependency |
|------|--------|------------|
| Per-muscle deformation offsets (visible contraction bulging) | 16 hrs | [DD003](DD003_Muscle_Model_Architecture.md) activation data |
...
| Dual Quaternion Skinning upgrade (if LBS artifacts visible) | 8 hrs | Stage B working |
...
| **Total** | | | **~3.2 ms** |
| Spine shortcut (Phase 3) | Spline eval per vertex | ~1.6M | 0.8 ms |
...
|-------|----------|----------|--------|-------|
| SPH elastic particle positions (per frame) | [DD001](DD001_Body_Physics_Architecture.md) (Sibernetic) | Per-particle (x, y, z) | OME-Zarr: `body/positions/` or direct from Sibernetic state | µm |
| SPH elastic particle rest positions | [DD001](DD001_Body_Physics_Architecture.md) (Sibernetic) | Per-particle (x, y, z) at t=0 | Snapshot | µm |
| Virtual Worm mesh files (static) | Blender2NeuroML repo | OBJ per cell/tissue | OBJ mesh files | µm |
| Muscle activation (optional, Phase 5) | [DD003](DD003_Muscle_Model_Architecture.md) | Per-muscle [0,1] | OME-Zarr: `muscle/activation/` | dimensionless |
| Pharynx pumping state (optional, Phase 5) | [DD007](DD007_Pharyngeal_System_Architecture.md) | Per-section [0,1] | OME-Zarr: `pharynx/pumping_state/` | dimensionless |

**Outputs (What This Subsystem Produces)**

| Output | Consumer DD | Variable | Format | Units |
|--------|------------|----------|--------|-------|
| Deformed mesh vertex positions (per frame) | [DD012](DD012_Dynamic_Visualization_Architecture.md) viewer (rendering) | Per-vertex (x, y, z) for all 688 meshes | GPU buffer (WebGPU `GPUBuffer`) | µm |
| Deformed mesh normals (per frame) | [DD012](DD012_Dynamic_Visualization_Architecture.md) viewer (lighting) | Per-vertex normal vector | GPU buffer | unit vectors |
| Collision debug overlay (optional) | [DD012](DD012_Dynamic_Visualization_Architecture.md) viewer (debug mode) | Per-vertex boolean (collided/not) | GPU buffer | boolean |
...
visualization:
  ...
  deformation_method: "cage"         # "spine" (Phase 3) or "cage" (Phase 4+)
...
|-------------|----|-----------------------------|
| SPH particle positions | [DD001](DD001_Body_Physics_Architecture.md) | If particle count, indexing, or coordinate frame changes, bone clusters and skin weights must be recomputed |
...
| Muscle activation (Phase 5) | [DD003](DD003_Muscle_Model_Architecture.md) | If activation range or muscle ID mapping changes, per-muscle deformation offsets must be updated |
| OME-Zarr data format | [DD012](DD012_Dynamic_Visualization_Architecture.md) | If Zarr schema changes, data loading in the deformation pipeline must update |

| Depends On Me | DD | What Breaks If I Change |
|---------------|----|-----------------------------|
| Visual rendering | [DD012](DD012_Dynamic_Visualization_Architecture.md) / [DD012.1](DD012.1_Visual_Rendering_Specification.md) | If deformed vertex buffer format changes, the renderer must update its vertex attribute bindings |
| Cell selection / inspector | [DD012](DD012_Dynamic_Visualization_Architecture.md) | If mesh-to-cell-ID mapping changes, click-to-identify must update |
| Activity coloring | [DD012.1](DD012.1_Visual_Rendering_Specification.md) | Activity color overlay is applied to deformed mesh vertices — deformation must preserve vertex-to-cell mapping (vertex indices must not change) |
...
3. Implement spine-based deformation (Phase 3 — get meshes moving)
