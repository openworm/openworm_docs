...
**Note:** Backend stabilization Issues 40–44 in [DD011_draft_issues](DD011_draft_issues.md) are also labeled `DD001` and are cross-referenced here but not duplicated.
...
- **DD Section to Read:** [DD001 — How to Build & Test](DD001_Body_Physics_Architecture.md#how-to-build-test) (Step 3) and [DD001 Quality Criteria](DD001_Body_Physics_Architecture.md#quality-criteria) (criterion 1)
- **Depends On:** None
...
- **Sponsor Summary Hint:** The basic health check for any SPH simulation — did the physics blow up? NaN values mean the computation diverged (division by zero, impossible forces). Escaped particles mean the simulation lost containment. This script is listed as a DD001 deliverable but was never created. The existing `owPhysicTest.cpp` already checks energy conservation — this extends that logic into a comprehensive Python stability checker.
...
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, physics
- **DD Section to Read:** [DD001 — How to Build & Test](DD001_Body_Physics_Architecture.md#how-to-build-test) (Step 4) and [DD001 Quality Criteria](DD001_Body_Physics_Architecture.md#quality-criteria) (criterion 2)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Document existing binary test configurations and create missing scenario directories`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** physics
- **DD Section to Read:** [DD001 Quality Criteria](DD001_Body_Physics_Architecture.md#quality-criteria) (criterion 4) and [DD001 Backend Stabilization Roadmap — Cross-Backend Parity Requirements](DD001_Body_Physics_Architecture.md#cross-backend-parity-requirements)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Generate and save OpenCL baseline metrics for cross-backend parity tests`
...
- **Title:** `[DD001] Port existing SPH kernel unit tests to run on PyTorch backend`
- **Labels:** `DD001`, `ai-workable`, `L2`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, physics
- **DD Section to Read:** [DD001 Quality Criteria](DD001_Body_Physics_Architecture.md#quality-criteria) (criterion 3, 5) and [DD001 — SPH Kernel Functions](DD001_Body_Physics_Architecture.md#sph-kernel-functions)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Port existing SPH kernel unit tests to run on Taichi backend`
- **Labels:** `DD001`, `ai-workable`, `L2`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, physics
...
**Note:** The core backend stabilization issues are tracked in [DD011_draft_issues.md](DD011_draft_issues.md) as Issues 39–42 (labeled `DD001`). They cover:
...
Stability validation scripts (`check_stability.py`, `validate_incompressibility.py`) are DD001 Issues 1–2 above.

The issues below supplement that sequence with DD001-specific work.

---

### Issue 7: Document OpenCL kernel architecture (`sphFluid.cl`)

- **Title:** `[DD001] Document OpenCL kernel architecture for algorithmic audit`
- **Labels:** `DD001`, `human-expert`, `L2`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** opencl, physics, sph
- **DD Section to Read:** [DD001 Backend Stabilization Roadmap — The Result Quality Gap](DD001_Body_Physics_Architecture.md#the-result-quality-gap) and [DD001 — Implementation References](DD001_Body_Physics_Architecture.md#implementation-references)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Add PyTorch backend smoke test to GitHub Actions CI`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** ci-cd, python
- **DD Section to Read:** [DD001 Backend Stabilization Roadmap — Stabilization Sequence](DD001_Body_Physics_Architecture.md#stabilization-sequence) (step 6)
...
### Issue 9: Audit physical parameters in code vs. DD001 spec

- **Title:** `[DD001] Audit physical parameters in Sibernetic code against DD001 specification`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, physics
- **DD Section to Read:** [DD001 — Physical Parameters](DD001_Body_Physics_Architecture.md#physical-parameters)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Approach:** Create — no parameter audit exists, but `owPhysicsConstant.h` is well-documented and provides the code-side values to compare against DD001.
- **Files to Modify:**
    - None (research issue — output is a summary posted on the issue)
- **Test Commands:**
    - N/A (audit task)
- **Acceptance Criteria:**
    - [ ] For each parameter in DD001's Physical Parameters table, find its value in the codebase
    - [ ] Document: file path, line number, variable name, actual value
    - [ ] Flag any discrepancies between DD001 spec and code
...
- **Sponsor Summary Hint:** DD001 specifies exact physical parameters (rest density 1000 kg/m³, viscosity 4e-6 Pa·s, etc.) but does the code actually use these values? And do all three backends use the same values? Parameter drift is a silent source of cross-backend divergence. This audit finds any mismatches. The good news: `owPhysicsConstant.h` has extensive inline documentation, making the audit tractable.

---

### Issue 10: Benchmark all backends (performance comparison)

- **Title:** `[DD001] Benchmark all backends: OpenCL vs. PyTorch vs. Taichi (Metal/CUDA)`
- **Labels:** `DD001`, `human-expert`, `L2`
- **Roadmap Phase:** Phase A1
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, benchmarking
- **DD Section to Read:** [DD001 Backend Stabilization Roadmap — Stabilization Sequence](DD001_Body_Physics_Architecture.md#stabilization-sequence) (step 7)
...
- **Acceptance Criteria:**
    ...
    - [ ] Validates the "~3x faster" (Taichi Metal) and "~5x faster" (Taichi CUDA) claims from DD001
- **Sponsor Summary Hint:** The DD001 backends table claims Taichi is 3-5x faster than OpenCL, but these are target estimates. This issue produces actual measured benchmarks on real hardware — the data needed to decide which backend to recommend for which platform.
...
- **DD Section to Read:** [DD001 — Deliverables](DD001_Body_Physics_Architecture.md#deliverables) (OME-Zarr rows) and [DD012](DD012_Dynamic_Visualization_Architecture.md) (OME-Zarr schema)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **DD Section to Read:** [DD001 — Deliverables](DD001_Body_Physics_Architecture.md#deliverables) (surface mesh row) and [DD001 — How to Visualize](DD001_Body_Physics_Architecture.md#how-to-visualize) (surface mesh description)
...
- **DD Section to Read:** [DD001 — Integration Contract — Configuration](DD001_Body_Physics_Architecture.md#configuration) and [DD011 §1](DD011_Simulation_Stack_Architecture.md#1-simulation-configuration-system-openwormyml) (`simulation.output_interval`)
...
- **DD Section to Read:** [DD001 — Alternatives Considered — FEM](DD001_Body_Physics_Architecture.md#1-finite-element-method-fem) (Update 2026-02 section)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Acceptance Criteria:**
    ...
    - [ ] Document: muscle actuator interface — `Muscle.cpp` implements 96 actuators; verify mapping compatibility with DD003's 96-muscle activation array
...
- **Title:** `[DD001] Create Python bindings for Sibernetic C++ library`
- **Labels:** `DD001`, `human-expert`, `L3`
- **Roadmap Phase:** Phase 2+
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, c++, pybind11
- **DD Section to Read:** [DD001 — Integration Contract](DD001_Body_Physics_Architecture.md#integration-contract) (coupling dependencies)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Verify and document gel/agar environment support in openworm.yml`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Phase 2+
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** physics
- **DD Section to Read:** [DD001 — Boundaries](DD001_Body_Physics_Architecture.md#boundaries-explicitly-out-of-scope) (item 3: gel simulation) and Palyanov et al. 2018 (Section 2c, agar gel)
...
- **Files to Modify:**
    ...
    - Documentation update in DD001 (if gel support is confirmed working)
- **Test Commands:**
    - `./build/Sibernetic -f configuration/worm_crawl_demo`
- **Acceptance Criteria:**
    ...
    - [ ] Document environment options in DD001: liquid (swimming, `sphFluid.cl`) vs. gel (crawling, `sphFluid_crawling.cl`)
...
- **DD Section to Read:** [DD001 — Technical Approach](DD001_Body_Physics_Architecture.md#technical-approach) and [DD001 — Implementation References](DD001_Body_Physics_Architecture.md#implementation-references)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Sponsor Summary Hint:** New contributors need a map before they can navigate. This document explains what each file does, how data flows through the simulation, and what happens in a single timestep. DD001 is the specification (what should happen); this is the implementation guide (where the code lives and how it works). The existing 17.5KB README and well-documented `owPhysicsConstant.h` provide a strong foundation.

---

### Issue 18: Document muscle cell mapping (96 units to particle indices)

- **Title:** `[DD001] Document muscle cell mapping from 96 units to elastic particle indices`
- **Labels:** `DD001`, `ai-workable`, `L2`
- **Roadmap Phase:** Any
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** docs, physics
- **DD Section to Read:** [DD001 — Muscle Actuation](DD001_Body_Physics_Architecture.md#muscle-actuation-force-injection) and Palyanov et al. 2018 (Section 2b)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Acceptance Criteria:**
    ...
    - [ ] PR checklist from DD001 Quality Criteria (all 6 criteria)
    ...
    - [ ] Links to DD001 for specifications and DD011 for Docker integration
...
- **Title:** `[DD001] Create annotated changelog documenting Sibernetic's evolution`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Any
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** git, docs
- **DD Section to Read:** [DD001 — Validated Kinematic Outputs](DD001_Body_Physics_Architecture.md#validated-kinematic-outputs-palyanov-et-al-2018)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Title:** `[DD001] Audit existing Sibernetic test suite and document test coverage`
- **Labels:** `DD001`, `ai-workable`, `L1`
- **Roadmap Phase:** Any
- **Target Repo:** `openworm/Sibernetic`
- **Required Capabilities:** python, testing
- **DD Section to Read:** [DD001 Quality Criteria](DD001_Body_Physics_Architecture.md#quality-criteria) (criterion 3: unit tests)
- **Depends On:** None
- **Existing Code to Reuse:**
...
- **Acceptance Criteria:**
    ...
    - [ ] Identify gaps: which DD001 Quality Criteria are not covered by existing tests
...
- **Sponsor Summary Hint:** DD001 says "76+ tests pass" on PyTorch, but which tests? Before adding new tests, we need to know what exists. This audit catalogs the current test suite, identifies gaps, and creates a roadmap for improving coverage. You can't improve what you don't measure.
...
### Cross-Reference: DD011 Backend Stabilization Issues (also labeled DD001)

| DD011 Issue | Title | Label | Level |
|-------------|-------|-------|-------|
| 39 | Create cross-backend parity test suite | `DD001`, `ai-workable` | L2 |
| 40 | Fix Taichi elastic coordinate-space bug | `DD001`, `human-expert` | L2 |
| 41 | Audit PyTorch/Taichi result quality gap | `DD001`, `human-expert` | L3 |
| 42 | Graduate backends to Stable/Production | `DD001`, `ai-workable` | L2 |

**Combined DD001 total (this doc + DD011):** 25 issues
